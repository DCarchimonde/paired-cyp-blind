...
2026-09-08 的 AutoDL 日志已确认上述两个真实 Lightning 自检通过。
恢复第一项任务时，旧比较逻辑又因预测表列数不同而停止。
已上传的同任务历史文件确认测试输入为 86 行，列为
`Molecule_Name, SMILES, CYP3A4_is_TDI`；训练导出只有 `SMILES, CYP3A4_is_TDI`。
Chemprop 的独立预测会保留输入标识列，再替换目标列为概率；训练内的导出使用 SMILES 和目标列。
依据为 [独立预测保存实现](https://github.com/chemprop/chemprop/blob/v2.3.1/chemprop/cli/predict.py)
和 [训练预测保存实现](https://github.com/chemprop/chemprop/blob/v2.3.1/chemprop/cli/train.py)。
前一版模拟测试没有覆盖这两种实际格式，直接比较整表是审计代码的缺陷。

更新后恢复与最终复验共用同一比较函数：必须包含全部指定目标，只允许额外出现
已知的 `Molecule_Name` 标识列；逐行对照冻结输入核验 SMILES 以及存在的标识列，
检查概率有限且在 [0,1]，再逐目标比较概率。没有取任意公共列、排序、重排样本或放宽数值容差。
恢复记录和日志现在分别保存 val/test 的列名、行数、标识检查情况及最大概率差。
原文件不被改写；先前失败的恢复目录保留。仍用同一启动命令，核验通过后才显示 `RECOVERED PASS`。
...
本地 22 项针对性测试通过，覆盖 PRC 方向、last 保存语义、错误元数据/权重拒绝、
来源登记、禁止训练输入/包装器/协议/依赖变更、恢复预测一致性、工件保留和未知耗时，
以及两种真实导出格式、缺失目标、未知/重复列、错行/错标识、非有限/越界概率和实际数值不一致的拒绝。
恢复候选到完整复验的测试使用模拟权重依赖；不代表本地运行了 Torch/GPU。
另用已上传的 75 份历史测试预测（50 单任务、25 多任务，共 6,200 行）
保留原概率并补回输入元数据，验证格式兼容性；这项检查没有重新进行模型推断。
此前新选模审计在实际上传的全部 75 条曲线上均拒绝错误的最小 PRC 选择，
45 个 TDI 分区的独立只读审计通过。用户最新日志已证实 AutoDL 上原 200 任务检查
与真实 PRC 方向/早停自检通过；9 月 8 日日志进一步证实 last 生命周期真实自检通过，
最佳/last 保存 epoch 均为 1，最终训练 epoch 为 2，且预测恢复最佳权重。
当前审查环境未安装 Torch，也没有用户的 4090；第一项实际模型的恢复仍须由更新入口
在 AutoDL 执行。尚未生成完整的 75 项修正结果。
